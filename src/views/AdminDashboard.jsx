...
export default function AdminDashboard({ onLogout }) {
  ...
  const [newCategory, setNewCategory] = useState({ nombre: '', descripcion: '', tipo_cubo: '' });
  const [newRecord, setNewRecord] = useState({
    nombre_completo: '', edad: '', pais: '', red_social: '',
    tiempo_segundos: '', fecha_registro: new Date().toISOString().split('T')[0]
  });
  ...
  const fetchRecordsByCategory = async (catId) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/records/category/${catId}`);
      setRecords(res.data);
    } catch (err) {
  ...
  const handleCreateCategory = async (e) => {
    e.preventDefault();
    setError(''); setSuccess('');
    if (!newCategory.nombre || !newCategory.tipo_cubo) {
      setError('Nombre y Tipo de Cubo son obligatorios.');
      return;
    }
    ...
    try {
      ...
      setNewCategory({ nombre: '', descripcion: '', tipo_cubo: '' });
      fetchCategories();
    } catch (err) {
  ...
  const handleCreateRecord = async (e) => {
    ...
    if (!newRecord.nombre_completo || !newRecord.edad || !newRecord.pais || !newRecord.tiempo_segundos) {
      setError('Por favor, llena todos los campos obligatorios del competidor y su tiempo.');
      return;
    }
    ...
    try {
      ...
      await axios.post(`http://localhost:5000/api/records/category/${selectedCategory}/compose`, {
        ...newRecord,
      ...
      setSuccess('Competidor y Récord registrados correctamente.');
      setNewRecord({ nombre_completo: '', edad: '', pais: '', red_social: '', tiempo_segundos: '', fecha_registro: new Date().toISOString().split('T')[0] });
      fetchRecordsByCategory(selectedCategory);
    } catch (err) {
      setError(err.response?.data?.message || 'Error al insertar el récord compuesto.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteRecord = async (idRecord) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/records/${idRecord}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="bg-brand-bg min-h-screen text-white font-sans flex flex-col justify-between">
      <nav className="bg-brand-dark px-6 py-4 flex justify-between items-center border-b border-gray-800">
        <div className="flex items-center space-x-3">
          <div className="bg-brand-accent h-5 w-5 rounded-sm animate-pulse"></div>
          <span className="text-lg font-black tracking-wider uppercase text-white">SpeedCuber OS <span className="text-brand-accent font-mono text-xs">[ADMIN]</span></span>
        </div>
        <button 
          onClick={onLogout}
      ...
      <div className="max-w-7xl w-full mx-auto px-6 mt-4">
        {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-xs p-3 rounded-lg font-medium">⚠️ {error}</div>}
      ...
      <main className="max-w-7xl w-full mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6 flex-grow">
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="bg-brand-dark border border-gray-800 rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleCreateCategory} className="space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-gray-400">Nombre de Categoría</label>
                <input 
                  type="text" 
                  value={newCategory.nombre}
                  onChange={(e) => setNewCategory({...newCategory, nombre: e.target.value})}
                  placeholder="Ej. 3x3x3 Blindfolded" 
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-gray-400">Tipo de Cubo</label>
                <input 
                  type="text" 
                  value={newCategory.tipo_cubo}
                  onChange={(e) => setNewCategory({...newCategory, tipo_cubo: e.target.value})}
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  ...
                  placeholder="Detalles sobre las reglas del reto..." 
          ...
          <div className="bg-brand-dark border border-gray-800 rounded-2xl p-6 shadow-xl space-y-3">
            <h3 className="text-sm font-black tracking-widest text-gray-300 uppercase">Categorías Activas</h3>
        ...
        <div className="space-y-6 lg:col-span-2 flex flex-col justify-between">
          ...
          <div className="bg-brand-dark border border-gray-800 rounded-2xl p-6 shadow-xl space-y-4">
            <h3 className="text-sm font-black tracking-widest text-brand-accent uppercase">Inscribir Competidor y Registrar Récord</h3>
            ...
            <form onSubmit={handleCreateRecord} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-gray-400">Red Social o WCA ID <span className="text-gray-600">(Opcional)</span></label>
                <input 
                  type="text" 
                  value={newRecord.red_social}
                  onChange={(e) => setNewRecord({...newRecord, red_social: e.target.value})}
                  placeholder="wca.link/202X..." 
                  className="w-full bg-slate-900/60 border border-gray-700 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-accent"
                />
              </div>
              ...
              <div className="sm:col-span-2 pt-2">
                ...
                >
                  {loading ? 'Insertando en BD...' : 'Asignar y Guardar Récord Compuesto'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-brand-dark border border-gray-800 rounded-2xl p-6 shadow-xl space-y-3 flex-grow mt-6">
            <h3 className="text-sm font-black tracking-widest text-gray-300 uppercase">Monitoreo de Récords Activos</h3>
                  ...
                  {records.length === 0 ? (
                    ...
                    records.map((rec) => (
                      <tr key={rec.id_record} className="hover:bg-slate-900/40 transition-colors">
                        <td className="p-3 font-bold text-white">{rec.nombre_completo} <span className="text-[10px] text-gray-500 font-normal">({rec.edad} años)</span></td>
